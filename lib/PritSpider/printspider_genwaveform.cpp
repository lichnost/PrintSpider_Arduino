...
void /* IRAM_ATTR */ printspider_fire_nozzle_color(uint8_t *l, int p, int color) {
...
void /* IRAM_ATTR */ printspider_fire_nozzle_black(uint8_t *l, int p, int row) {
	if (row) p+=168;
	int j=p/14;
	int k=13-(p%14);
...
int /* IRAM_ATTR */ printspider_generate_waveform(uint16_t *w, const uint16_t *tp, const uint8_t *nozdata, int l) {
